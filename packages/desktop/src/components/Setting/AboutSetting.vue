...
import type { UpdateLogType } from "@core/types/main";
import { getUpdateLog, openLink } from "@core/utils/helper";
...
const getContributors = async () => {
  try {
    const response = await fetch(
      "https://api.github.com/repos/xiaoleng-ros/ToneSolo/contributors?per_page=100&anon=true",
    );
    ...
    if (Array.isArray(data)) {
      const list = data
        ...
        .map((item: any) => ({
          ...
          role: item.login === "xiaoleng-ros" ? "Owner / Full Stack" : "Contributor",
          url: item.html_url || "",
