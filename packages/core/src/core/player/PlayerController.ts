import { useBlobURLManager } from "@core/core/resource/BlobURLManager";
import { useDataStore, useMusicStore, useSettingStore, useStatusStore } from "@core/stores";
import { type SongType } from "@core/types/main";
import { RepeatModeType, ShuffleModeType } from "@core/types/shared";
import { PlaybackStatus } from "@core/types/smtc";
import { calculateLyricIndex } from "@core/utils/calc";
import { getCoverColor } from "@core/utils/color";
import { isElectron } from "@core/utils/env";
import { getPlayerInfoObj, getPlaySongData } from "@core/utils/format";
import { handleSongQuality, shuffleArray } from "@core/utils/helper";
import lastfmScrobbler from "@core/utils/lastfmScrobbler";
import { calculateProgress } from "@core/utils/time";
...
class PlayerController {
  ...
  public async nextOrPrev(
    ...
    if (nextIndex >= playListLength) {
      if (statusStore.repeatMode === "off" && autoEnd) {
        statusStore.playStatus = false;
        return;
      }
      nextIndex = 0;
